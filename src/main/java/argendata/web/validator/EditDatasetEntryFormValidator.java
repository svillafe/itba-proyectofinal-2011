...
@Component
public class EditDatasetEntryFormValidator implements Validator{
	...
	@Autowired
	public EditDatasetEntryFormValidator(DatasetService datasetService){
	...
	public void validate(Object arg0, Errors err) {
		...
		if(dForm.getPublisher()==null || dForm.getPublisher().isEmpty()){
			err.rejectValue("title", "DatasetForm.invalid.publisher","El Publicante es obligatorio.");
		}
		
		if(dForm.getPublisher()!=null && dForm.getPublisher().length()>140){
			err.rejectValue("title", "DatasetForm.tooLong.publisher","El Publicante es muy largo.");
		}
	
		if(dForm.getPublisher()!=null && dForm.getPublisher().contains(":")){
			err.rejectValue("title", "DatasetForm.tooLong.publisher","El Publicante no pueden contener ':'.");
		}
		
		if(dForm.getAccessURL()==null || dForm.getAccessURL().isEmpty()){
			err.rejectValue("title", "DatasetForm.invalid.accesURL","La dirección de acceso es obligatorio.");
		}
		...
		if(dForm.getAccessURL()!=null && dForm.getAccessURL().length()>500){
			err.rejectValue("title", "DatasetForm.invalid.accesURL","La dirección de acceso supera los 500 caracteres.");
		}
		
		if(dForm.getKeyword()==null || dForm.getKeyword().isEmpty()){
			err.rejectValue("title", "DatasetForm.invalid.keyword","Al menos proponga una palabra clave.");
		}
